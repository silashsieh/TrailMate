...
│   ├── TelemetryFrame.swift           # high-frequency telemetry-plane value type (position + route derivatives)
...
```
┌──────────────────────────────────────────────────────────┐
│  SwiftUI chrome over one imperative MKMapView            │  Presentation
│  surface (ContentView: Sidebar, MapArea,                 │
│  PlaybackProgress, VirtualJoystickView, sheets).         │
│  MapArea hosts MapSurface: MKMapView, imperative         │
│  camera, telemetry-fed dot — a position tick             │
│  evaluates no SwiftUI body.                              │
├──────────────────────────────────────────────────────────┤
...
│    deviation check, then a frequency-split push:         │
│    telemetry frames from the tick path, capped 5 Hz     │
│    active); the change-guarded bridge snapshot           │
│    throttled (2 Hz playback / 10 Hz active).             │
...
|`SimulationActor`     |Engines (nav/joystick/integrator/noise), aggregator + idle-jitter loops, deviation check, dual-plane publish (renewable latest-wins telemetry stream + change-guarded bridge snapshot), App Nap token. **One per session** — independent actors; the only cross-session shared object is the injected `RecorderService` (until epic 040 gives each session its own).|
...
The engines are marked `nonisolated final class` so the simulation actor can call them synchronously inside a tick — no per-tick `await` hop. Every push feeds **two planes** (epic 041). The high-frequency plane is a per-actor latest-wins `AsyncStream<TelemetryFrame>` (`bufferingNewest(1)`) carrying the clean position and its route-relative derivatives — leg progress, elapsed distance, route deviation, recording point count — consumed by `MapSurfaceController` (not SwiftUI), which slides each session's dot annotation in place without evaluating a SwiftUI body; latest-wins means a consumer that wakes late reads the freshest frame, not a backlog. The stream is **single-consumer and renewable**: `SimulationActor.telemetryStream()` hands out a fresh stream per call (retiring any prior subscription's continuation and seeding the newest frame), so a consumer that is cancelled and re-created — the map surface torn down on window close and rebuilt on reopen in menu-bar background mode — resubscribes and frames flow again. A stored app-lifetime stream would instead be dead after that first cancel, leaving the reopened map frozen. The structural plane is the `@Observable` `SimulationStateBridge` the SwiftUI sidebar readouts observe; its `apply` is change-guarded per field — a property is written only when the incoming value differs, and the coordinate guard compares lat/lon since `CLLocationCoordinate2D` is not `Equatable` — because Observation does not dedupe same-value writes, so an unguarded 2–10 Hz push would re-invalidate every observing view every tick even when nothing it reads changed. The two planes are fed at *different* cadences (epic 037): the telemetry plane yields from the tick path, capped at 5 Hz during route playback (`SimulationTiming.mapTelemetryInterval`, 200 ms) while joystick/active motion publishes at the 10 Hz active cadence for hand-steering responsiveness — every frame forces a MapKit render pass, and the 2026-07-11 Release A/B measured uncapped 10 Hz costing more during playback (21–22% CPU) than the pre-migration map, while 5 Hz beats every pre-migration figure (14.8/13.6%) and stays 2.5× smoother than the old 2 Hz; state transitions (teleport/stop), mid-tick playback-state flips (a route completing between ticks), and the trailing edge of motion all publish unthrottled, so terminal coordinates and playing→idle transitions are never dropped by the leading-edge cap — while the structural bridge push keeps its own throttle, **2 Hz during playback** and 10 Hz for active non-playing motion, so the SwiftUI sidebar readouts never rise to 10 Hz during playback. Both planes derive from the same snapshot for a given tick, so they can never disagree about the position they publish. The backend still receives every SETQ tick at 10 Hz because `setLocationQuiet` is `nonisolated` on `DaemonBridge` (cached pipe handle + serial queue). A `Thread.sleep(forTimeInterval:)` on MainActor will *not* delay SETQ delivery — that was the failure mode the actor split eliminated. `DeviceSession` exposes `routeVersion`, a monotonic counter bumped on every `routeCoordinates` assignment, so a consumer can diff route identity in O(1) rather than comparing coordinate arrays.
