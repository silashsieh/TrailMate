...
1. Sparkle bootstrap/update (epics 038 and 047): install the published v2.1.3 DMG on a clean Mac, confirm **Check for Updates…** reports no newer release, then publish v2.2.0 and verify the same command downloads, validates, installs, and relaunches into v2.2.0. Before publishing, confirm the signed dry run validates the exported app, staged copy, and exact app mounted from its finished DMG. Also confirm automatic-check/download preferences persist, a deliberately tampered archive is rejected, and the manual GitHub DMG remains usable if Pages is unavailable. v2.1.2 cannot self-update and must be replaced manually.
