...
- **Issue-driven changes earn a `phases.md` entry.** When a user-filed issue is resolved, append a new `## Phase N — <title> (issue #X, YYYY-MM-DD)` section at the tail of [docs/project-plan/phases.md](docs/project-plan/phases.md) with **Status:** and a paragraph describing what shipped and why. Phase numbering is monotonic; match the format of the previous entry (Phase 12 for issue #2, Phase 13 for issue #3). (Stated 2026-05-28.)
