...
1. Sparkle bootstrap/update (epic 038): install the published v2.1.2 DMG on a clean Mac, confirm **Check for Updates…** reports no newer release, then publish v2.2.0 and verify the same command downloads, validates, installs, and relaunches into v2.2.0. Also confirm automatic-check/download preferences persist, a deliberately tampered archive is rejected, and the manual GitHub DMG remains usable if Pages is unavailable.
